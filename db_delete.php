...
list($batch_id, $row, $col) = explode('_', $p);

// discard clone id part from the batch_id
$batch_id = substr($batch_id, 0, 2);
...
if (is_numeric($row) && is_numeric($col)) {
	...
	sqlQuery("delete from redips_timetable where batch_id='$batch_id' and tbl_row=$row and tbl_col=$col limit 1");
